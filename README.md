...
Core queues the notification before your workflow gets a chance to close anything, and nothing downstream reconsiders it. `app/Listeners/SendNotificationToUsers.php` only bails out on `STATUS_SPAM`; neither `Subscription::usersToNotify()` nor the queued `SendNotificationToUsers` job re-reads the status. Core says so plainly at `app/Subscription.php:16`:
...
Someone [asked for this upstream](https://github.com/freescout-help-desk/freescout/issues/5112) in December 2025, proposing a workflow action called "Remove from notifications" for exactly this use case. It was closed the same day with a pointer to the feature-request board, and never built. [#3740](https://github.com/freescout-help-desk/freescout/issues/3740) is the same complaint from the auto-reply direction, answered with "There is no such feature for now."
...
Replies, assignments, notes, and conversations you closed by hand are untouched. So is everything else if the Workflows module isn't installed; with no pseudo-user to match, the module goes inert rather than erroring.

The bias is deliberate. A missing conversation, a malformed event list, a database error: every one of them falls through to *notify*, and gets logged on the way. Over-notifying is a nuisance. Under-notifying hides real tickets.

## Requirements

FreeScout with the [Workflows module](https://freescout.net/module/workflows/). Without it this module does nothing at all, deliberately so: `module.json` does **not** declare `requiredModules: {"workflows": ...}`, because that would stop this module from *installing* without Workflows rather than letting it install and stay inert. Don't add that field. It would break the degrade-to-no-op behaviour described above.

Verified against **FreeScout 1.8.232**, which `module.json`'s `requiredAppVersion` also names. Every official module (Workflows, SpamFilter, MobileNotifications, Mentions) uses that field the same way, so core can warn an admin before activation on an older install instead of letting the module silently do nothing. What it actually depends on is the `subscription.subscriptions` and `subscription.users_to_notify` filters plus `Conversation.closed_by_user_id`, all long-standing core; a test pins the README line and `requiredAppVersion` together so the two can't drift apart unnoticed.

The module's own code is conservative PHP with no version-specific syntax. `composer.json` declares `^8.2` because that's what CI covers (8.2 through 8.5, the last being what current FreeScout images ship); it's a development constraint, not an install requirement, and nothing evaluates it at install time.

## Install

Updates work the same way no matter how you install this. FreeScout core checks `latestVersionUrl` for every non-official module on each load of **Manage → Modules**, and shows an **Update Now** button when `module.json`'s declared version is newer than what's installed. That's `app/Http/Controllers/ModulesController.php`'s `modules()` method plus `App\Module::updateModule()`, which downloads and extracts `latestVersionZipUrl` in place; both verified against core rather than inferred. No Module Manager, no license key, nothing module-specific: any third-party module carrying those two `module.json` fields updates this way. Module Manager's own role is narrower, covering the part stock core has no UI for at all: installing a module from a GitHub URL in the first place.
...
The directory name **must** be `QuietAutoClosed`. FreeScout maps `Modules\ => Modules/`, and `module.json` declares `Modules\QuietAutoClosed\Providers\QuietAutoClosedServiceProvider`, so any other name is a class-not-found on activation. (The lowercase `quietautoclosed` is the *alias*, a different thing entirely; it's the key in the modules table.)

There is nothing to build and no dependencies to install; the module has no runtime `vendor/`.

Then restart your PHP worker, since some images only register modules at boot, and activate the module under **Manage → Modules**. No license key: it's third-party and MIT-licensed.
...
Then clear the cache with `php artisan freescout:clear-cache`, or the cached config still lists the provider and the module keeps booting. Delete the directory to remove it entirely.
...
The three coverage uploads (Codecov, Codacy, Qlty's coverage step) are **reporting, not gating**. Each one is guarded on its token being present, so a pull request from a fork skips it rather than failing, and each is `continue-on-error`. The gate is the in-repo threshold, which needs no network and no account; an unreachable dashboard should never be the reason a correct change can't merge. Qlty's separate static-analysis scan isn't a coverage upload and isn't exempted by any of this. See the row above.

Every GitHub Action is pinned to a full commit SHA rather than a tag, and each job starts with StepSecurity's Harden-Runner in audit mode. This repo builds an artifact that installs itself onto other people's helpdesks, so its build pipeline counts as part of the security surface. [SECURITY.md](SECURITY.md) covers the rest.

## Known limits

**It silences the in-app notifications menu too**, not just email and push. That's usually what people want, but an auto-closed ticket won't show up in the bell menu either. It's still sitting in the mailbox, closed and tagged, exactly as the workflow left it.

In practice only conversations created by customers are affected. When an agent creates a conversation in the UI or through the API, core defers the workflow to a background job while registering the notification immediately, so the workflow hasn't closed anything yet at the moment recipients get picked. Nothing is suppressed. That fails open, which is the right direction, but don't expect coverage on that path.

Version 1.0 hardcodes the rule. Want *workflow-tagged* instead of *workflow-closed*, a per-mailbox opt-out, or a settings toggle? Open an issue. It's the obvious next step, and I'd rather build it around a real use case than guess at one.

Two smaller things. The workflow user is identified by email (`fsworkflow@example.org`, the Workflows module's own constant), so if that ever changes upstream, suppression stops and notifications resume rather than vanish. And decisions are memoized per notification pass, cleared on `subscription.process_events` — under a persistent worker model like Octane, which reuses a process across requests, call `QuietAutoClosedServiceProvider::flushCache()` between them. Stock FreeScout runs php-fpm and per-invocation artisan commands, so this doesn't come up.
